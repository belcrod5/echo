...
function buildAiTools(mcp, rawTools, hooks = {}) {
    ...
    for (const t of filteredTools) {
        // MCP からの inputSchema をベースにする（なければデフォルト）
        ...
        // 念のため、object 以外はスキップ（将来の変なツール対策）
        if (schema.type && schema.type !== "object") {
            console.warn(`[MCPClient] Skip tool "${t.name}" because inputSchema.type is not 'object':`, schema.type);
            continue;
        }

        aiTools[t.name] = tool({
            description: t.description ?? "",
            // ★ v5 では parameters ではなく inputSchema を使う
...
class ApiClient {
    ...
    setupSignalHandlers() {
        const cleanup = async () => {
            console.log('\n[ApiClient] Received termination signal, cleaning up...');
            await this.cleanup();
            process.exit(0);
        };
        ...
        process.on('beforeExit', async () => {
            console.log('[ApiClient] Process is about to exit, cleaning up...');
            await this.cleanup();
        });
    }
    ...
    async init() {
        ...
        /*----------------  設定ファイル読込 ----------------*/
        const llmConfigPath = path.join(SETTINGS_DIR, "llm_configs_api.json");
        ...
        switch (provider) {
            ...
            case "openai": {
                ...
                if (this.llm_configs.baseURL) {
                    openAiProviderOptions.baseURL = this.llm_configs.baseURL;
                }
                const openai = createOpenAI(openAiProviderOptions);
                const options = {};
                if (this.llm_configs.reasoning_effort) {
    ...
    async processQueryStream(
        ...
        // --- DEBUG: Inspect browser_close schema ---
        const debugTool = allTools.find(t => t.name === "browser_close");
        if (debugTool) {
            console.log("\n[DEBUG] browser_close raw definition:");
            console.dir(debugTool, { depth: null, colors: true });
        }
        // -------------------------------------------
